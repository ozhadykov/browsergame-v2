...
export default class Game {
    ...
    this.level = new Level({
      ...
      background: new BaseElement({
        ...
        imageCropBox: new Box({
          ...
          gameScaleY: this.gameScaleY,
          y: 5,
          height: this.gameScreen.getCanvas().height * this.gameScaleY,
          width: this.gameScreen.getCanvas().width * this.gameScaleX,
        }),
        framesX: 1,
        framesY: 1,
      }),
    });
    console.log(this.level.getBackground())
  ...
  tick() {
    if (
      !this.player.keys.pause.pressed &&
      !this.goal.checkForGoalReached(this.player)
    ) {
      this.ctx.save();

      //finde das kleinere besser aber nur mein geschmack. Falls geändert müssen wir auch den Sprung entsprechend anpassen
      this.ctx.scale(2 * this.gameScaleX, 2 * this.gameScaleY);
      ...
      this.ctx.fillRect(
        0,
        0,
        this.gameScreen.getCanvas().clientWidth,
        this.gameScreen.getCanvas().clientHeight
      );
      ...
      this.ctx.translate(-this.cameraBox.getX(), -this.cameraBox.getY());
 ...
 */
  ...
  drawjumpChargingBar() {
    for (
      let i = 0;
      i <= this.player.maxJumpCharge;
      i += this.player.maxJumpCharge / 10
    ) {
      // auch mit /20; /100 möglich
      if (Date.now() - this.player.chargingJumpTime >= i) {
        ...
        charginBarCtx.fillRect(
          0,
          charginBarCanvas.clientHeight -
            charginBarCanvas.clientHeight * (i / this.player.maxJumpCharge),
          charginBarCanvas.clientWidth,
          charginBarCanvas.clientHeight / 10
        );
      }
    }
  }
